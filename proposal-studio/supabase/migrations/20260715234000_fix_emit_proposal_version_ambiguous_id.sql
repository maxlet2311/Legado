-- Fix: `emit_proposal_version` fallaba con "column reference \"id\" is
-- ambiguous". Causa: `RETURNS TABLE(id uuid, ...)` declara `id` como variable
-- de salida de la función; las subconsultas de client/brand usaban `id` sin
-- calificar en su cláusula WHERE, y PL/pgSQL (variable_conflict = error, el
-- default) no puede decidir si es la variable de salida o la columna de la
-- tabla. Se corrige calificando explícitamente con alias de tabla.

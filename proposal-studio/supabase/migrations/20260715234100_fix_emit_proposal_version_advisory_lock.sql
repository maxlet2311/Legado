-- Fix: `select ... order by version_number desc limit 1 for update` sobre
-- `proposal_versions` devolvía cero filas (found=false) en combinación con su
-- política RLS, aun existiendo la fila — reproducido de forma aislada con
-- `_debug_check_last_version` (mismo SELECT con y sin FOR UPDATE, mismo
-- usuario, misma fila: sin FOR UPDATE la encuentra, con FOR UPDATE no).
-- Causaba que dos emisiones concurrentes sin cambios reales no se
-- deduplicaran y chocaran contra el unique index (proposal_id, version_number).
--
-- Se reemplaza el `for update` sobre la fila por un advisory lock
-- transaccional por proposal_id (`pg_advisory_xact_lock`), que no depende de
-- RLS ni de filas existentes y serializa exactamente igual (se libera solo al
-- terminar la transacción). El resto de la lógica de dedupe queda igual.
...
begin
  ...
  -- Serializa emisiones concurrentes de la MISMA propuesta. Se libera solo al
  -- terminar la transacción (commit o rollback de esta llamada al RPC).
  ...
  from (
    ...
    from public.brands br where br.id = v_proposal.brand_id
  ) b;
...
comment on function public.emit_proposal_version(uuid) is
  'Emite un snapshot inmutable de la propuesta. Serializado por pg_advisory_xact_lock(proposal_id); dedupe por checksum de content_json: doble clic o sin cambios reales devuelve is_new=false con la versión existente.';
